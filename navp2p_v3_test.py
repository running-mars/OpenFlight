...
class SimLoop:
    def __init__(self,
        ...
        self.goal_position = [-300., -400., 180.]     # meters in x, y, z
        self.reach_radius = 30.
        self.max_flight_time = 100.
    ...
    def _setup_flight(self):
        ...
        self.data_path = './flight_data_' + self.episode_init_time + '.csv'
    ...
    def navigate(self):
        ...
        while True:
            ...
            goal_horiz = np.sqrt((self.goal_position[1] - position[1]) ** 2 +
                                 (self.goal_position[0] - position[0]) ** 2)
            ...
            # control with Api
            self.ap.roll_hold(action[0] * np.pi)
            self.ap.pitch_hold(action[1] * np.pi * 0.5)
            self.ap.airspeed_hold_w_throttle((20 + 6 * action[2]) * 1.94)

            # for data safe
            if any(abs(element) > 10 for element in v_ang_radps):
                break
...
if __name__ == '__main__':
    ...
    simLoop = SimLoop()
    simLoop._setup_flight()
    simLoop.navigate()
    simLoop.simIO.flight_visualization(csv_file=simLoop.data_path)
